 ...
 * @param workspaceRootPath - Absolute path to workspace folder (e.g., ~/.iweather/workspaces/xxx)
 ...
 * (like iweather-docs) that don't have filesystem folders.
 */
export function getSourcesBySlugs(workspaceRootPath: string, slugs: string[]): LoadedSource[] {
  ...
  for (const slug of slugs) {
    ...
    if (isBuiltinSource(slug)) {
      // Currently only iweather-docs is a builtin source
      if (slug === 'iweather-docs') {
 ...
 * Built-in sources (like iweather-docs) are always available and merged
...
export async function createSource(
  ...
  // (bundled guides removed - agent should search iweather-docs MCP for service-specific guidance)
