 ...
 *
 * Stores credentials in an encrypted file at ~/.iweather/credentials.enc
...
// File location
const CREDENTIALS_DIR = join(homedir(), '.iweather');
...
export class SecureStorageBackend implements CredentialBackend {
  ...
  private getEncryptionKey(salt: Buffer): Buffer {
    ...
    const stableMachineId = createHash('sha256')
      ...
      .update('iweather-v2') // Bumped version for new key derivation
      .digest();
