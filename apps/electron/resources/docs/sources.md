...
**Before doing anything else**, search for a specialized guide using the iweather-docs MCP:

```
mcp__iweather-docs__SearchIWeather({ query: "{service} source setup" })
```
...
Agent: [FIRST: Searches for Linear guide]
       mcp__iweather-docs__SearchIWeather({ query: "linear source setup" })
...
- `~/.iweather/workspaces/{workspaceId}/sources/{sourceSlug}/`
...
`~/.iweather/provider-domains.json`
...
1. Create the source folder:
   ```bash
   mkdir -p ~/.iweather/workspaces/{ws}/sources/my-source
   ```
