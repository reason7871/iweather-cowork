...
3. **Preset themes**: `~/.iweather/themes/{name}.json` - Complete theme packages
4. **Theme overrides**: `~/.iweather/theme.json` - Override specific colors (app-level)
...
```
~/.iweather/workspaces/{id}/config.json
```
...
Create `~/.iweather/theme.json` to override specific colors:
...
Preset themes are complete theme packages stored at `~/.iweather/themes/`. Each preset is a JSON file with theme colors and metadata.
...
2. Save it to `~/.iweather/themes/{name}.json`
...
1. Create `~/.iweather/theme.json` for overrides or `~/.iweather/themes/{name}.json` for a preset
...
- Check file is in correct location (`~/.iweather/theme.json` for overrides, `~/.iweather/themes/` for presets)
