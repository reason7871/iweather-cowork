...
export default function InputSettingsPage() {
  ...
  return (
    <div className="h-full flex flex-col">
      <PanelHeader title={t('inputPage.title')} actions={<HeaderMenu route={routes.view.settings('input')} />} />
      <div className="flex-1 min-h-0 mask-fade-y">
        <ScrollArea className="h-full">
          <div className="px-5 py-7 max-w-3xl mx-auto">
            <div className="space-y-8">
              {/* Typing Behavior */}
              <SettingsSection title={t('inputPage.typing')} description={t('inputPage.typingDesc')}>
                <SettingsCard>
                  <SettingsToggle
                    label={t('inputPage.autoCapitalisation')}
                    description={t('inputPage.autoCapitalisationDesc')}
                    checked={autoCapitalisation}
                    onCheckedChange={handleAutoCapitalisationChange}
                  />
                  <SettingsToggle
                    label={t('inputPage.spellCheck')}
                    description={t('inputPage.spellCheckDesc')}
                    checked={spellCheck}
              ...
              {/* Send Behavior */}
              <SettingsSection title={t('inputPage.sending')} description={t('inputPage.sendingDesc')}>
                <SettingsCard>
                  <SettingsMenuSelectRow
                    label={t('inputPage.sendMessageWith')}
                    description={t('inputPage.sendMessageWithDesc')}
                    value={sendMessageKey}
                    onValueChange={handleSendMessageKeyChange}
                    options={[
                      { value: 'enter', label: t('inputPage.enter'), description: t('inputPage.enterDesc') },
                      { value: 'cmd-enter', label: isMac ? t('inputPage.cmdEnter') : t('inputPage.ctrlEnter'), description: t('inputPage.cmdEnterDesc') },
                    ]}
                  />
                </SettingsCard>
              </SettingsSection>
