...
export default function LabelsSettingsPage() {
  ...
  return (
    <div className="h-full flex flex-col">
      <PanelHeader title={t('labelsPage.title')} actions={<HeaderMenu route={routes.view.settings('labels')} />} />
          ...
          <div className="px-5 py-7 max-w-3xl mx-auto">
            <div className="space-y-8">
              {isLoading ? (
                ...
                <>
                  {/* About Section */}
                  <SettingsSection title={t('labelsPage.aboutLabels')}>
                    <SettingsCard className="px-4 py-3.5">
                      <div className="text-sm text-muted-foreground leading-relaxed space-y-1.5">
                        <p>
                          {t('labelsPage.aboutLabelsDesc1')}
                        </p>
                        <p>
                          {t('labelsPage.aboutLabelsDesc2')}
                        </p>
                        <p>
                          {t('labelsPage.aboutLabelsDesc3')}
                        </p>
                        <p>
                          <button
                            type="button"
                  ...
                  <SettingsSection
                    title={t('labelsPage.labelHierarchy')}
                    description={t('labelsPage.labelHierarchyDesc')}
                    action={
                      <EditPopover
                        trigger={<EditButton />}
                    ...
                    <SettingsCard className="p-0">
                      {labels.length > 0 ? (
                        <LabelsDataTable
                          ...
                          maxHeight={350}
                          fullscreen
                          fullscreenTitle={t('labelsPage.labelHierarchy')}
                        />
                      ) : (
                        <div className="p-8 text-center text-muted-foreground">
                          <p className="text-sm">{t('labelsPage.noLabelsConfigured')}</p>
                          <p className="text-xs mt-1 text-foreground/40">
                            {t('labelsPage.noLabelsHint')}
                          </p>
                        </div>
                      )}
                    </SettingsCard>
                  ...
                  <SettingsSection
                    title={t('labelsPage.autoApplyRules')}
                    description={t('labelsPage.autoApplyRulesDesc')}
                    action={
                      <EditPopover
                        trigger={<EditButton />}
                  ...
                  >
                    <SettingsCard className="p-0">
                      <AutoRulesDataTable
                        ...
                        maxHeight={350}
                        fullscreen
                        fullscreenTitle={t('labelsPage.autoApplyRules')}
                      />
                    </SettingsCard>
                  </SettingsSection>
                </>
