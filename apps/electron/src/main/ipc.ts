...
import { readFileAttachment, perf, validateImageForClaudeAPI, IMAGE_LIMITS } from '@iweather/shared/utils'
import { safeJsonParse } from '@iweather/shared/utils/files'
import { getPreferencesPath, getSessionDraft, setSessionDraft, deleteSessionDraft, getAllSessionDrafts, getWorkspaceByNameOrId, addWorkspace, setActiveWorkspace, loadStoredConfig, saveConfig, type Workspace, getLlmConnections, getLlmConnection, addLlmConnection, updateLlmConnection, deleteLlmConnection, getDefaultLlmConnection, setDefaultLlmConnection, touchLlmConnection, isCompatProvider, isAnthropicProvider, isOpenAIProvider, isCopilotProvider, getDefaultModelsForConnection, getDefaultModelForConnection, type LlmConnection, type LlmConnectionWithStatus, getGitBashPath, setGitBashPath, clearGitBashPath } from '@iweather/shared/config'
import { getSessionAttachmentsPath, validateSessionId } from '@iweather/shared/sessions'
import { loadWorkspaceSources, getSourcesBySlugs, type LoadedSource } from '@iweather/shared/sources'
import { isValidThinkingLevel } from '@iweather/shared/agent/thinking-levels'
import { getCredentialManager } from '@iweather/shared/credentials'
import { AppServerClient, getCodexPath } from '@iweather/shared/codex'
import type { ModelDefinition } from '@iweather/shared/config'
...
async function fetchAndStoreCopilotModels(slug: string, accessToken: string): Promise<void> {
  ...
  const writeDebugFile = async () => {
    try {
      const debugPath = join(homedir(), '.iweather', 'copilot-debug.log')
...
export function registerIpcHandlers(sessionManager: SessionManager, windowManager: WindowManager): void {
  ...
  ipcMain.handle(IPC_CHANNELS.CHECK_WORKSPACE_SLUG, async (_event, slug: string) => {
    const defaultWorkspacesDir = join(homedir(), '.iweather', 'workspaces')
  ...
  ipcMain.handle(IPC_CHANNELS.GET_RELEASE_NOTES, () => {
    const { getCombinedReleaseNotes } = require('@iweather/shared/release-notes') as typeof import('@iweather/shared/release-notes')
    return getCombinedReleaseNotes()
  })

  ipcMain.handle(IPC_CHANNELS.GET_LATEST_RELEASE_VERSION, () => {
    const { getLatestReleaseVersion } = require('@iweather/shared/release-notes') as typeof import('@iweather/shared/release-notes')
  ...
  ipcMain.handle(IPC_CHANNELS.UPDATE_DISMISS, async (_event, version: string) => {
    const { setDismissedUpdateVersion } = await import('@iweather/shared/config')
    setDismissedUpdateVersion(version)
  })

  // Get dismissed version
  ipcMain.handle(IPC_CHANNELS.UPDATE_GET_DISMISSED, async () => {
    const { getDismissedUpdateVersion } = await import('@iweather/shared/config')
  ...
  ipcMain.handle(IPC_CHANNELS.LOGOUT, async () => {
    try {
      ...
      const configPath = join(homedir(), '.iweather', 'config.json')
  ...
  ipcMain.handle(IPC_CHANNELS.WORKSPACE_SETTINGS_GET, async (_event, workspaceId: string) => {
    ...
    const { loadWorkspaceConfig } = await import('@iweather/shared/workspaces')
  ...
  ipcMain.handle(IPC_CHANNELS.WORKSPACE_SETTINGS_UPDATE, async (_event, workspaceId: string, key: string, value: unknown) => {
    ...
    const { loadWorkspaceConfig, saveWorkspaceConfig } = await import('@iweather/shared/workspaces')
    const config = loadWorkspaceConfig(workspace.rootPath)
    if (!config) {
  ...
  ipcMain.handle(IPC_CHANNELS.LLM_CONNECTION_TEST, async (_event, slug: string): Promise<{ success: boolean; error?: string }> => {
    try {
      ...
      if (isOpenAiProvider && connection.authType === 'oauth') {
        ...
        try {
          const { refreshChatGptTokens } = await import('@iweather/shared/auth/chatgpt-oauth')
      ...
      if (isAnthropicProvider && connection.authType === 'oauth') {
        const { getValidClaudeOAuthToken } = await import('@iweather/shared/auth/state')
  ...
  ipcMain.handle(IPC_CHANNELS.CHATGPT_START_OAUTH, async (_event, connectionSlug: string): Promise<{
    ...
    try {
      const { startChatGptOAuth, exchangeChatGptCode } = await import('@iweather/shared/auth')
      const credentialManager = getCredentialManager()
  ...
  ipcMain.handle(IPC_CHANNELS.CHATGPT_CANCEL_OAUTH, async (): Promise<{ success: boolean }> => {
    try {
      const { cancelChatGptOAuth } = await import('@iweather/shared/auth')
      cancelChatGptOAuth()
  ...
  ipcMain.handle(IPC_CHANNELS.COPILOT_START_OAUTH, async (event, connectionSlug: string): Promise<{
    ...
    try {
      const { startGithubOAuth } = await import('@iweather/shared/auth')
      const credentialManager = getCredentialManager()
  ...
  ipcMain.handle(IPC_CHANNELS.COPILOT_CANCEL_OAUTH, async (): Promise<{ success: boolean }> => {
    try {
      const { cancelGithubOAuth } = await import('@iweather/shared/auth')
      cancelGithubOAuth()
  ...
  // Create a new source
  ipcMain.handle(IPC_CHANNELS.SOURCES_CREATE, async (_event, workspaceId: string, config: Partial<import('@iweather/shared/sources').CreateSourceInput>) => {
    const workspace = getWorkspaceByNameOrId(workspaceId)
    if (!workspace) throw new Error(`Workspace not found: ${workspaceId}`)
    const { createSource } = await import('@iweather/shared/sources')
  ...
  ipcMain.handle(IPC_CHANNELS.SOURCES_DELETE, async (_event, workspaceId: string, sourceSlug: string) => {
    const workspace = getWorkspaceByNameOrId(workspaceId)
    if (!workspace) throw new Error(`Workspace not found: ${workspaceId}`)
    const { deleteSource } = await import('@iweather/shared/sources')
  ...
  ipcMain.handle(IPC_CHANNELS.SOURCES_START_OAUTH, async (_event, workspaceId: string, sourceSlug: string) => {
    try {
      ...
      const { loadSource, getSourceCredentialManager } = await import('@iweather/shared/sources')
  ...
  ipcMain.handle(IPC_CHANNELS.SOURCES_GET_PERMISSIONS, async (_event, workspaceId: string, sourceSlug: string) => {
    ...
    const { getSourcePermissionsPath } = await import('@iweather/shared/agent')
  ...
  ipcMain.handle(IPC_CHANNELS.WORKSPACE_GET_PERMISSIONS, async (_event, workspaceId: string) => {
    ...
    // Load raw JSON file (not normalized) for UI display
    const { existsSync, readFileSync } = await import('fs')
    const { getWorkspacePermissionsPath } = await import('@iweather/shared/agent')
  ...
  // Get default permissions from ~/.iweather/permissions/default.json
  ...
  ipcMain.handle(IPC_CHANNELS.DEFAULT_PERMISSIONS_GET, async () => {
    const { existsSync, readFileSync } = await import('fs')
    const { getAppPermissionsDir } = await import('@iweather/shared/agent')
  ...
  ipcMain.handle(IPC_CHANNELS.SOURCES_GET_MCP_TOOLS, async (_event, workspaceId: string, sourceSlug: string) => {
    ...
    try {
      ...
      const { CraftMcpClient } = await import('@iweather/shared/mcp')
      ...
      const { loadSourcePermissionsConfig, permissionsConfigCache } = await import('@iweather/shared/agent')
  ...
  ipcMain.handle(IPC_CHANNELS.SEARCH_SESSIONS, async (_event, workspaceId: string, query: string, searchId?: string) => {
    ...
    const { getWorkspaceSessionsPath } = await import('@iweather/shared/workspaces')
  ...
  ipcMain.handle(IPC_CHANNELS.SKILLS_GET_FILES, async (_event, workspaceId: string, skillSlug: string) => {
    ...
    const { getWorkspaceSkillsPath } = await import('@iweather/shared/workspaces')
  ...
  ipcMain.handle(IPC_CHANNELS.SKILLS_DELETE, async (_event, workspaceId: string, skillSlug: string) => {
    ...
    const { deleteSkill } = await import('@iweather/shared/skills')
  ...
  ipcMain.handle(IPC_CHANNELS.STATUSES_LIST, async (_event, workspaceId: string) => {
    ...
    const { listStatuses } = await import('@iweather/shared/statuses')
  ...
  ipcMain.handle(IPC_CHANNELS.STATUSES_REORDER, async (_event, workspaceId: string, orderedIds: string[]) => {
    ...
    const { reorderStatuses } = await import('@iweather/shared/statuses')
  ...
  ipcMain.handle(IPC_CHANNELS.LABELS_LIST, async (_event, workspaceId: string) => {
    ...
    const { listLabels } = await import('@iweather/shared/labels/storage')
    return listLabels(workspace.rootPath)
  })

  // Create a new label in a workspace
  ipcMain.handle(IPC_CHANNELS.LABELS_CREATE, async (_event, workspaceId: string, input: import('@iweather/shared/labels').CreateLabelInput) => {
    const workspace = getWorkspaceByNameOrId(workspaceId)
    if (!workspace) throw new Error('Workspace not found')

    const { createLabel } = await import('@iweather/shared/labels/crud')
  ...
  ipcMain.handle(IPC_CHANNELS.LABELS_DELETE, async (_event, workspaceId: string, labelId: string) => {
    ...
    const { deleteLabel } = await import('@iweather/shared/labels/crud')
  ...
  ipcMain.handle(IPC_CHANNELS.VIEWS_LIST, async (_event, workspaceId: string) => {
    ...
    const { listViews } = await import('@iweather/shared/views/storage')
    return listViews(workspace.rootPath)
  })

  // Save views (replaces full array)
  ipcMain.handle(IPC_CHANNELS.VIEWS_SAVE, async (_event, workspaceId: string, views: import('@iweather/shared/views').ViewConfig[]) => {
    const workspace = getWorkspaceByNameOrId(workspaceId)
    if (!workspace) throw new Error('Workspace not found')

    const { saveViews } = await import('@iweather/shared/views/storage')
  ...
  ipcMain.handle(IPC_CHANNELS.THEME_GET_WORKSPACE_COLOR_THEME, async (_event, workspaceId: string) => {
    const { getWorkspaces } = await import('@iweather/shared/config/storage')
    const { getWorkspaceColorTheme } = await import('@iweather/shared/workspaces/storage')
  ...
  ipcMain.handle(IPC_CHANNELS.THEME_SET_WORKSPACE_COLOR_THEME, async (_event, workspaceId: string, themeId: string | null) => {
    const { getWorkspaces } = await import('@iweather/shared/config/storage')
    const { setWorkspaceColorTheme } = await import('@iweather/shared/workspaces/storage')
    const workspaces = getWorkspaces()
    const workspace = workspaces.find(w => w.id === workspaceId)
    if (!workspace) return
  ...
  ipcMain.handle(IPC_CHANNELS.TOOL_ICONS_GET_MAPPINGS, async () => {
    const { getToolIconsDir } = await import('@iweather/shared/config/storage')
    const { loadToolIconConfig } = await import('@iweather/shared/utils/cli-icon-resolver')
    const { encodeIconToDataUrl } = await import('@iweather/shared/utils/icon-encoder')
    const { join } = await import('path')
  ...
  ipcMain.handle(IPC_CHANNELS.LOGO_GET_URL, async (_event, serviceUrl: string, provider?: string) => {
    const { getLogoUrl } = await import('@iweather/shared/utils/logo')
  ...
  ipcMain.handle(IPC_CHANNELS.NOTIFICATION_GET_ENABLED, async () => {
    const { getNotificationsEnabled } = await import('@iweather/shared/config/storage')
  ...
  ipcMain.handle(IPC_CHANNELS.NOTIFICATION_SET_ENABLED, async (_event, enabled: boolean) => {
    const { setNotificationsEnabled } = await import('@iweather/shared/config/storage')
  ...
  ipcMain.handle(IPC_CHANNELS.INPUT_GET_AUTO_CAPITALISATION, async () => {
    const { getAutoCapitalisation } = await import('@iweather/shared/config/storage')
  ...
  ipcMain.handle(IPC_CHANNELS.INPUT_SET_AUTO_CAPITALISATION, async (_event, enabled: boolean) => {
    const { setAutoCapitalisation } = await import('@iweather/shared/config/storage')
    setAutoCapitalisation(enabled)
  })

  // Get send message key setting
  ipcMain.handle(IPC_CHANNELS.INPUT_GET_SEND_MESSAGE_KEY, async () => {
    const { getSendMessageKey } = await import('@iweather/shared/config/storage')
  ...
  ipcMain.handle(IPC_CHANNELS.INPUT_SET_SEND_MESSAGE_KEY, async (_event, key: 'enter' | 'cmd-enter') => {
    const { setSendMessageKey } = await import('@iweather/shared/config/storage')
    setSendMessageKey(key)
  })

  // Get spell check setting
  ipcMain.handle(IPC_CHANNELS.INPUT_GET_SPELL_CHECK, async () => {
    const { getSpellCheck } = await import('@iweather/shared/config/storage')
  ...
  ipcMain.handle(IPC_CHANNELS.INPUT_SET_SPELL_CHECK, async (_event, enabled: boolean) => {
    const { setSpellCheck } = await import('@iweather/shared/config/storage')
    setSpellCheck(enabled)
  })

  // Get keep awake while running setting
  ipcMain.handle(IPC_CHANNELS.POWER_GET_KEEP_AWAKE, async () => {
    const { getKeepAwakeWhileRunning } = await import('@iweather/shared/config/storage')
  ...
  ipcMain.handle(IPC_CHANNELS.POWER_SET_KEEP_AWAKE, async (_event, enabled: boolean) => {
    const { setKeepAwakeWhileRunning } = await import('@iweather/shared/config/storage')
  ...
  ipcMain.handle(IPC_CHANNELS.APPEARANCE_GET_RICH_TOOL_DESCRIPTIONS, async () => {
    const { getRichToolDescriptions } = await import('@iweather/shared/config/storage')
  ...
  ipcMain.handle(IPC_CHANNELS.APPEARANCE_SET_RICH_TOOL_DESCRIPTIONS, async (_event, enabled: boolean) => {
    const { setRichToolDescriptions } = await import('@iweather/shared/config/storage')
